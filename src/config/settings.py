"""Application configuration for Password Reset Agent."""
...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="password-reset-agent", description="Application name")
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5432/password_reset_agent",
    ...
    opensearch_index: str = Field(default="password_reset_agent_vectors", description="Index")
    ...
    mcp_server_name: str = Field(default="password-reset-agent-mcp", description="MCP server name")

    a2a_discovery_url: str = Field(default="http://localhost:8500", description="A2A discovery URL")
    a2a_agent_id: str = Field(default="password-reset-agent", description="A2A agent ID")
